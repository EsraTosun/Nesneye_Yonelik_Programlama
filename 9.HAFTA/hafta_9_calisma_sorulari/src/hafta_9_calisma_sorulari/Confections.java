...
public class Confections extends Urun implements IUrun
{
			...
			ArrayList<GrainsCereals> GrainsCerealslist, int kategori, int oran,String islemYapilacakUrun) 
	{
		...
		for(int i=0;i<Confectionslist.size();i++)
 		{  
			if(Confectionslist.get(i).Adi.compareTo(islemYapilacakUrun)==0)
		{
				Confectionslist.get(i).BirimAgirligi+="+"+oran+"+"+3;
		}
	}
	     if(kontrol==0)
		System.out.println("urun bulunamadi");
			...
			double BirimFiyatı,String Detay) 
	{
		...
		for(int i=0;i<Confectionslist.size();i++)
		{
			toplamStok+=Confectionslist.get(i).StokMiktari;
		}
		int ortalamaStok=toplamStok/Confectionslist.size();
		
		Confections.ListeyiYazdir(Confectionslist);
		Confections confections=new Confections(Adi, kategori, BirimAgirligi, BirimFiyatı, ortalamaStok,
				"Confections", Detay,kategori);
		Confectionslist.add(0,confections);
		Confections.ListeyiYazdir(Confectionslist);
